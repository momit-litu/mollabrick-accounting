...
switch ($q){
	case "insert_or_update":
		if(isset($category_id) && $category_id == ""){
			...
			$return = $dbClass->insert("expense_head", $columns_value);
			if($return){ 
				echo "1";
			}else "0";
		}
			...
			$return = $dbClass->update("expense_head",$columns_value, $condition_array);
			if($return) echo "2";
			else        echo "0";			 
		}
	break;
	...
	case "delete_category":
		...
		if($delete_permission==1){
			...
			$return = $dbClass->delete("expense_head", $condition_array);
		}
		if($return==1) echo "1";
		else 		   echo "0";
	break;
