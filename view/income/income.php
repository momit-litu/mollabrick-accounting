    ...
    <div class="x_content">
       ...
       <div style="height:250px; width:100%; overflow-y:scroll">
        <table id="expense_Table" name="table_records" class="table table-bordered  responsive-utilities jambo_table table-striped  table-scroll ">
            <thead>
                <tr class="headings">
                    <th class="column-title" >Income Head</th>
...
<div class="x_panel expense_entry_cl">
    <div class="x_title">
        ...
        <ul class="nav navbar-right panel_toolbox">
            <li>
				<a class="collapse-link" id="toggle_form"><button class="btn btn-default btn-info collapse-link-btn">Add Income</button><i class="fa fa-chevron-down"></i></a>
            </li>
        </ul>
        <div class="clearfix"></div>
    </div>
    <div class="x_content" id="iniial_collapse">
		...
		<form id="expense_form" name="expense_form" enctype="multipart/form-data" class="form-horizontal form-label-left">   
			<div class="row">
				...
				<div class="form-group " id="agreement_div" style="display:none" >
                    <label class="control-label col-md-2 col-sm-2 col-xs-12">Renter<span class="required">*</span></label>
                    <div class="col-md-6 col-sm-6 col-xs-12">
						<select  id="agreement_id" name="agreement_id" required class="form-control col-lg-12" > 
...
$(document).ready(function () {	
	...
	$("#head_name").autocomplete({
		...
		select: function(event, ui) { 
			...
			//get the renter agreements
			
			$.ajax({
				url: project_url+'controller/renterAgreementController.php',
				dataType: "json",
				type: "post",
				async:false,
				data: {
				...
				success: function(data){
					if(data.toal_records==0){
						//$('#project_id').val('');
						$('#amount').val('');
						$('#agreement_div').hide();
						$('#agreement_id').children('option:not(:first)').remove();
					}
					else{
						...
						records.forEach((agreement)=>{
							project = agreement.project_code;
							rent 	= agreement.current_rent;
							agreement_option += "<option value='"+agreement.id+"'>"+agreement.full_name+"</option>";
						});
						$('#project_id').val(project);
						$('#amount').val(rent);
						$('#agreement_div').show();
						$('#agreement_id').append(agreement_option);
...
$(document).ready(function () {		
	...
	clear_form = function clear_form(){			 
		...
		$('#agreement_id option:gt(0)').remove();		
	}
