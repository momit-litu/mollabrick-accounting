...
<div class="x_panel">
    ...
    <div class="x_content">
       ...
       <div style="height:50vh; width:100%; overflow-y:scroll">
        <table id="expense_Table" name="table_records" class="table table-bordered  responsive-utilities jambo_table table-striped  table-scroll ">
            <thead>
            <tr class="headings">
                <th class="column-title">Expense Head</th>
                ...
                <th class="column-title no-link last" width="70px"><span class="nobr"></span></th>
            </tr>
            </thead>
...
<div class="x_panel expense_entry_cl">
    <div class="x_title">
        <h2>Expenses Entry Form</h2>
        <ul class="nav navbar-right panel_toolbox">
            <li>
				<a class="collapse-link" id="toggle_form"><button class="btn btn-default btn-info collapse-link-btn">Add Expenses</button><i class="fa fa-chevron-down"></i></a>
            </li>
        </ul>
        <div class="clearfix"></div>
    </div>
		...
		<form id="expense_form" name="expense_form" enctype="multipart/form-data" class="form-horizontal form-label-left">   
			<div class="row">
				<div class="form-group">
					<label class="control-label col-md-2 col-sm-2 col-xs-12">Head Name<span class="required">*</span></label>
               ...
               <!-- <div class="form-group">
                ...
                <div class="form-group">
                    <label class="control-label col-md-2 col-sm-2 col-xs-12" >Save as deposit</label>
                    <div class="col-md-4 col-sm-4 col-xs-12">
                        <input type="checkbox" id="deposit" name="deposit" class="form-control col-lg-12"/>
                    </div>
                </div>
				<div class="form-group deposit"  style="display: none">
                    <label class="control-label col-md-2 col-sm-2 col-xs-12">Stakeholder</label>
                    <div class="col-md-6 col-sm-6 col-xs-12">
						<select  id="stakeholder" name="stakeholder"  class="form-control col-lg-12" > 
						<option value=''>Select Stakeholder</option>
						<?php 
							foreach($stakeholders as $stakeholder){
								echo "<option value='".$stakeholder['id']."'>".$stakeholder['name']."</option>";
							}
						?>
						</select>
                    </div>
		...
		$.ajax({
			...
			success: function(data){
				...
				if(!jQuery.isEmptyObject(records_array)){
					...
					var colums_array=["id*identifier*hidden","headName","project_name*hidden-xs","date*hidden-xs","note*hidden-xs","amount"];
...
*/
$(document).ready(function () {		
	var url = project_url+"controller/expensesIncomeController.php";	
	...
	$('#save_expense_info').click(function(event){		
		...
		else if($("#deposit").prop("checked")==true && $.trim($('#stakeholder').val()) == ""){	
			success_or_error_msg('#form_submit_error','danger',"Please select stakeholder","#stakeholder");			
		}		
		else{
